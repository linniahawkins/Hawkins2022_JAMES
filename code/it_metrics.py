...
def conditional_mutual_information(df, source, target, condition, bins1, bins2, reshuffle=0, ts='D'):
    ...
    H_y = shannon_entropy([y],[bins1])
    H_z = shannon_entropy([z],[bins2])
    H_xz = shannon_entropy([x, z],[bins2, bins2])
    H_yz = shannon_entropy([y, z],[bins1, bins2])
    H_xyz = shannon_entropy([x, y, z],[bins2, bins1, bins2])
    return H_xz + H_yz - H_z - H_xyz


def information_partitioning(df, source_1, source_2, target, bins1, bins2, reshuffle=0):
    if reshuffle == 1:
    ...
    H_s1 = shannon_entropy(df[source_1].values, [bins1])
    H_s2 = shannon_entropy(df[source_2].values, [bins2])
    mi_s1_s2 = mutual_information(df, source_1, source_2, bins1)
    mi_s1_tar = mutual_information(df, source_1, target, bins1)
    mi_s2_tar = mutual_information(df, source_2, target, bins2)
    mi_s1_tar_cs2 = conditional_mutual_information(df, source_1, target, source_2, bins1, bins2)
...
def cal_it_performance(data_i, target, target_o, source_1, source_2, nbins=10, norm=0, log=0, obs=0):
    ...
    if log == 0:
        ...
        bins2 = np.linspace(0, 1, nbins+1)
    elif log == 1:
        bins1 = np.logspace(0, 1, nbins+1)
        bins2 = np.linspace(0, 1, nbins+1)
    elif log == 2:
        bins1 = np.linspace(0, 1, nbins+1)
        bins2 = np.logspace(0, 1, nbins+1)
    else:
        bins1 = np.logspace(0, 1, nbins+1)
        bins2 = np.logspace(0, 1, nbins+1)
    ...
    total_information_o, unique_1_o, unique_2_o, redundant_o, synergistic_o = information_partitioning(data_i, source_1, source_2, target_o, bins1, bins2)
    total_information, unique_1, unique_2, redundant, synergistic = information_partitioning(data_i, source_1, source_2, target, bins1, bins2)
    ...
    if obs == 1:
        return H_tar_o, total_information_o, unique_1_o, unique_2_o, redundant_o, synergistic_o
    else:
